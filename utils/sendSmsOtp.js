...
// ── Fortius Infocom SMS Gateway — DLT-approved templates ──
const SMS_API_BASE = "https://smsfortius.work/V2/apikey.php";
// Corrected to match the approved sender ID actually confirmed working on this
// Fortius account ("SAAVIT") — the previous "saaviinf" value was not a valid
// DLT sender ID and would have caused silent delivery failures.
const SMS_SENDER_ID = "SAAVIT";

// Two approved templates — used for two different purposes:
//   LOGIN        → sent when an existing account logs in via OTP (vendor or staff)
//   REGISTRATION → sent when a new vendor is verifying their mobile number before self-registering
const TEMPLATES = {
  // Template #2 in Manage Template ("verification code1") — requested to replace
  // the old login template.
  login: {
    id: "1677100000000391223",
    buildMessage: (otp) =>
      `Your Mobile verification code is ${otp}. For your security, please do not share this code with anyone. SAAVI INFINET`,
  },
  registration: {
    id: "1607100000000379312",
    buildMessage: (otp) =>
