...
// const { findRestrictedAreaForPoint } = require("../services/geofenceService");
...
//       return res.status(400).json({ success: false, message: "Only applications in Draft or Sent Back status can be submitted ❌" });
//     }
...
//       return res.status(400).json({ success: false, message: "Application is not in the correct status for this action ❌" });
//     }
...
//       return res.status(400).json({ success: false, message: "Application is not at the survey stage ❌" });
//     }

//     // ── Geo location is now COMPULSORY for every survey (28-10 request) ──
//     if (lat === undefined || lat === null || lat === "" || lng === undefined || lng === null || lng === "") {
//       return res.status(400).json({ success: false, message: "Geo location (latitude/longitude) is required to submit the survey ❌" });
//     }

//     // ── No Feriwala Area Geofencing (NEW) ──
//     // HARD BLOCK: survey cannot be submitted at all if the officer's captured GPS
//     // location falls inside an active "No Feriwala Area" restricted geofence.
//     // This is the earliest and strictest checkpoint — the application never even
//     // reaches A.M.C. / ID-card stage for a restricted-area location.
//     const restrictedArea = await findRestrictedAreaForPoint(lat, lng);
//     if (restrictedArea) {
//       return res.status(400).json({
//         success: false,
//         isInsideRestrictedArea: true,
//         area: { id: restrictedArea._id, name: restrictedArea.name },
//         message: `⚠️ Survey cannot be submitted — this location falls inside "${restrictedArea.name}" (No Feriwala Area). Street vending is restricted here.`,
//       });
//     }
...
//       return res.status(400).json({ success: false, message: "Application is not at the A.M.C. decision stage ❌" });
//     }
...
//     //   pushHistory(application, "Certificate Issued", req.user, `Smart Card auto-generated on approval (payment skipped) — Certificate No: ${certificateNo}`);
...
//   application.status = "A.M.C. Approved";
//   pushHistory(application, "A.M.C. Approved", req.user, remarks);
//   // Certificate is now generated separately when the A.M.C. clicks
//   // "Generate Smart Card" — see generateIdCardByAmc below.
// } else if (decision === "Sent Back") {
...
//       return res.status(400).json({ success: false, message: "Application is not at the payment stage ❌" });
//     }
...
//     if (role === "citizen") {
//       return res.status(403).json({ success: false, message: "Citizens are not allowed to view vendor applications ❌" });
//     }
...
// // ═══════════════════════════════════════════════════════
// //  14) GENERATE ID CARD — A.M.C. issues the Smart Card straight after approving the
// //      application, WITHOUT payment (payment is future scope — recordPayment/amcDecision's
// //      old auto Payment-Pending step are left untouched above, just not wired in for now).
// //      Only works for applications the A.M.C. has already approved ("A.M.C. Approved").
// // ═══════════════════════════════════════════════════════
// exports.generateIdCardByAmc = async (req, res) => {
//   try {
//     const { applicationNo } = req.params;
//     const { remarks } = req.body;
...
//     // ── A.M.C. फक्त स्वतःच्या ward च्या applications साठी ID card generate करू शकतो ──
//     if (req.user.role === "A.M.C.") {
//       const wardCheck = checkWardAccess(req.user, application.ward);
//       if (!wardCheck.ok) return res.status(403).json({ success: false, message: wardCheck.message });
//     }

//     if (application.status === "Certificate Issued") {
//       return res.status(400).json({ success: false, message: "ID Card already generated for this application ❌" });
//     }

//     if (!["A.M.C. Approved", "Payment Pending"].includes(application.status)) {
//       return res.status(400).json({ success: false, message: `ID Card can only be generated for applications that are A.M.C. Approved or Payment Pending. Current status: ${application.status} ❌` });
//     }

//     // ── No Feriwala Area Geofencing (NEW) — SAFETY NET ──
//     // Survey submission already blocks this at the source, but re-check here as a
//     // second gate right before the Smart Card is actually issued, in case the
//     // survey's geo location was ever set through an older flow / direct edit.
//     const surveyLat = application.survey?.geoLocation?.lat;
//     const surveyLng = application.survey?.geoLocation?.lng;
//     if (surveyLat !== undefined && surveyLat !== null && surveyLng !== undefined && surveyLng !== null) {
//       const restrictedArea = await findRestrictedAreaForPoint(surveyLat, surveyLng);
//       if (restrictedArea) {
//         return res.status(400).json({
//           success: false,
//           isInsideRestrictedArea: true,
//           area: { id: restrictedArea._id, name: restrictedArea.name },
//           message: `⚠️ ID Card cannot be generated — surveyed location falls inside "${restrictedArea.name}" (No Feriwala Area) ❌`,
//         });
//       }
//     }

//     const note = remarks?.trim() || "ID Card generated by A.M.C. (payment skipped — future scope)";
...
//     };
//     application.status = "Certificate Issued";
//     pushHistory(application, "Certificate Issued", req.user, `${note} — Certificate No: ${certificateNo}`);
...
//     });
//   } catch (error) {
//     console.error("Generate ID Card (A.M.C.) Error:", error);
//     return res.status(500).json({ success: false, message: "Server Error ❌", error: error.message });
//   }
// };

// // ═══════════════════════════════════════════════════════
// //  15) BULK GENERATE ID CARDS — A.M.C. selects multiple "A.M.C. Approved" applications from
// //      a table and generates Smart Cards for all of them in one go, without payment.
// // ═══════════════════════════════════════════════════════
// exports.bulkGenerateIdCards = async (req, res) => {
//   try {
//     const { applicationNos } = req.body; // array of applicationNo strings

//     if (!Array.isArray(applicationNos) || applicationNos.length === 0) {
//       return res.status(400).json({ success: false, message: "applicationNos array is required ❌" });
//     }

//     const generated = [];
//     const skipped = [];

//     for (const applicationNo of applicationNos) {
//       const application = await VendorApplication.findOne({ applicationNo });

//       if (!application) {
//         skipped.push({ applicationNo, reason: "Not found" });
//         continue;
//       }
...
//         const wardCheck = checkWardAccess(req.user, application.ward);
//         if (!wardCheck.ok) {
//           skipped.push({ applicationNo, reason: "Different ward" });
//           continue;
//         }
//       }

//       if (!["A.M.C. Approved", "Payment Pending"].includes(application.status)) {
//         skipped.push({ applicationNo, reason: `Not eligible (status: ${application.status})` });
//         continue;
//       }

//       // ── No Feriwala Area Geofencing (NEW) — SAFETY NET for bulk generation too ──
//       const bulkSurveyLat = application.survey?.geoLocation?.lat;
//       const bulkSurveyLng = application.survey?.geoLocation?.lng;
//       if (bulkSurveyLat !== undefined && bulkSurveyLat !== null && bulkSurveyLng !== undefined && bulkSurveyLng !== null) {
//         const restrictedArea = await findRestrictedAreaForPoint(bulkSurveyLat, bulkSurveyLng);
//         if (restrictedArea) {
//           skipped.push({ applicationNo, reason: `Inside restricted area: ${restrictedArea.name}` });
//           continue;
//         }
//       }
...
//         "Certificate Issued",
//         req.user,
//         `Bulk ID Card generated by A.M.C. (payment skipped — future scope) — Certificate No: ${certificateNo}`
//       );

//       await application.save();
//       generated.push({ applicationNo, certificateNo });
//     }

//     return res.status(200).json({
//       success: true,
//       message: `Bulk ID Card generation finished — ${generated.length} generated, ${skipped.length} skipped ✅`,
...
//     // ── status आता comma-separated multiple values सुद्धा स्वीकारतो (उदा.
//     //    "A.M.C. Approved,Payment Pending") — VendorList.jsx (Smart Card page) आधीपासूनच
//     //    STATUS_GROUPS[status].join(",") पाठवत होता, पण backend फक्त exact single-value
//     //    match करत होता त्यामुळे grouped-status tabs वर चुकीचे (रिकामे) results यायचे. ──
//     if (status) {
...
//     // ── search — नाव / मोबाईल / application no वर case-insensitive match. VendorList.jsx
//     //    (Smart Card page) हा param पाठवत होताच, पण backend तो कधीच वापरत नव्हता — म्हणजे
//     //    search box टाईप केल्यावरही पूर्ण unfiltered list तशीच परत यायची. ──
//     if (search && search.trim()) {
//       const safe = search.trim().replace(/[.*+?^${}()|[\]\\]/g, "\\$&"); // regex-escape user input
//       const re = new RegExp(safe, "i");
//       filter.$and = (filter.$and || []).concat([
//         { $or: [{ "personal.fullName": re }, { "personal.mobile": re }, { applicationNo: re }] },
//       ]);
//     }
...
exports.getAllApplications = async (req, res) => {
  try {
    ...
    // ── hasCertificate=true — Dashboard's "Smart Card Disbursed" KPI needs a count of
    // applications that already have a certificate number, regardless of current status
    // (A.M.C. can issue the ID card without payment, so it isn't tied to one status). Used
    // with limit:1 so only the cheap countDocuments() total is needed — no documents
    // actually get transferred just to count them. ──
    if (req.query.hasCertificate === "true") {
      filter["certificate.certificateNo"] = { $exists: true, $ne: "" };
    }

    // ── since — ISO date string, restricts results to createdAt >= since. Lets pages like
    // the Dashboard trend chart fetch only "last N days" worth of applications instead of
    // pulling the entire collection just to bucket a handful of recent days. ──
    if (req.query.since) {
...
// ── Any status is allowed EXCEPT one that's already got a certificate — this can now
    ...
    // ── Same certificate-generation logic as the normal payment flow ──
    // const certificateNo = await generateCertificateNo(); // OLD — applicationId nasल्याने error throw hota
    const certificateNo = await generateCertificateNo(application._id);
