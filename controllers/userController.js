...
// ✅ SEND OTP — generate OTP and send via WhatsApp (existing, unchanged) + SMS (new template, via sendSmsOtp)
exports.sendOtp = async (req, res) => {
  try {
    const { mobileNo } = req.body;
    ...
    // ── Send on both channels — WhatsApp (original, unchanged) and SMS (new template) ──
    await Promise.allSettled([sendWhatsAppOtp(mobile, otp), sendSmsOtp(mobile, otp, "login")]);
