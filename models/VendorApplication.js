...
//     workingAddress: { type: String, default: "" }, // Working Address
//     currentAddress: { type: String, default: "" }, //current Address
...
//     workingAddress: { type: String, default: "" }, // NEW
...
// // ══════════════════════════════════════════════════════════
// //  PERFORMANCE INDEXES — vendor list / applications list / survey list / pending
// //  approval queue सर्व ठिकाणी ward + status + createdAt (sort) वापरून query होते,
// //  त्यामुळे हा सर्वात जास्त वापरला जाणारा compound index आहे. बाकीचे fields जिथे
// //  थेट lookup किंवा search केला जातो तिथे वेगळे single-field indexes दिले आहेत.
// //  (जुना कोड डिलीट केलेला नाही — फक्त हे नवीन indexes जोडले आहेत.) ──
// // ══════════════════════════════════════════════════════════
// vendorApplicationSchema.index({ ward: 1, status: 1, createdAt: -1 });
// vendorApplicationSchema.index({ status: 1, createdAt: -1 });
// vendorApplicationSchema.index({ createdAt: -1 });
// vendorApplicationSchema.index({ createdById: 1 });
// vendorApplicationSchema.index({ "personal.mobile": 1 });
// vendorApplicationSchema.index({ vendorId: 1 });
...
vendorApplicationSchema.index({ "personal.mobile": 1 });
vendorApplicationSchema.index({ vendorId: 1 });
// Dashboard's "Smart Card Disbursed" count filters on certificate.certificateNo existing —
// index it so that count stays fast even as the collection grows.
vendorApplicationSchema.index({ "certificate.certificateNo": 1 });
